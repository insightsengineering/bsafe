...
dimnames(array_prop_th) <- list(rownames(th_prop_NA),
...
dimnames(array_rate_th) <- list(rownames(th_rate_NA),
                                c("mean", "sd", "med", "cri025", "cri975", "ess"),
                                paste0("Run ", 1:n_runs))
...
for(scen in c(1:11)[-4]){
  ...
  for(run in 1:n_runs){
    ...
    ana_obj <- summary(rob_map_prop, quantile.type = 7, digits = deci, na.rm = TRUE,
    ...
    ana_obj <- summary(lik_prop, quantile.type = 7, digits = deci, na.rm = TRUE,
                       names = TRUE, type = c("quartiles"),
                       probs = c(crilb, 0.5, criub))
    ...
    post_prop <- RBesT::postmix(rob_map_prop, n = as.numeric(new_n),
                                r = as.numeric(new_r))


    ana_obj <- summary(post_prop, quantile.type = 7, digits = deci, na.rm = TRUE,
                       names = TRUE, type = c("quartiles"),
                       probs = c(crilb, 0.5, criub))
    ...
    rob_map_rate <- RBesT::robustify(fit_rate, weight = as.numeric(testing_list_rates[[scen + 1]]$parameters["rob_weight"]),
                                     mean = log(as.numeric(testing_list_rates[[scen + 1]]$parameters["rob_mean"])), sigma = 1)

    set.seed(seeds[run])
    rob_rate_exp <- exp(RBesT::rmix(rob_map_rate, 100000))
    rob_rate_log <- summary(rob_map_rate, quantile.type = 7, digits = deci, na.rm = TRUE,
                            names = TRUE, type = c("quartiles"),
                            probs = c(crilb, 0.5, criub))
    ...
    lik_ana <- summary(lik_rate, quantile.type = 7, digits = deci, na.rm = TRUE,
                       names = TRUE, type = c("quartiles"),
                       probs = c(crilb, 0.5, criub))
    ...
    post_rate_log <- summary(post_rate, quantile.type = 7, digits = deci, na.rm = TRUE,
                             names = TRUE, type = c("quartiles"),
                             probs = c(crilb, 0.5, criub))
