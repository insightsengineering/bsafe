...
SimTestData <- function(
  ...
  res <- array(data = NA, dim = c(nStudy, 5, 2),
               dimnames = list(STUDYID = c(1:nStudy),
  ...
  for(i in 1:nStudy){
    ...
    res_SimStudy[[i]] = SimStudy(nPat = SimStudy_nPat,
                                 hz = SimStudy_hz,
  ...
  if(prior_data_conflict == TRUE){
    ...
    res_SimStudy[[nStudy]] = SimStudy(nPat = SimStudy_nPat,
                                      hz = pdc_hz,
                                      dropout = SimStudy_dropout,
                                      accr = SimStudy_accr,
                                      accr_method = SimStudy_accr_method,
  ...
  res_df <- res_df[,c("STUDYID", "HIST", "ARM", "N",
                      "SAF_TOPIC", "N_WITH_AE", "TOT_EXP")]

  # Hotfix for ExtraInfo upload
  res_df$DOSE <- 999
  res_df$FREQ <- 999
  res_df$LENGTH <- 999
