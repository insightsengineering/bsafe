...
model_summary_display <- function(map_object, select_analysis,
  ...
  if (is.null(map_object)) {
    ...
    stats_mat_rate      <- data.frame(rbind(mcmc_desc(sample_rate_log),
                                            mcmc_desc(sample_rate_exp)),
                                      row.names = c("MAP Prior: log(hazard)",
                                                    "MAP Prior: hazard"))
...
summary_stats_robust_map_prior_display <- function(map_object, select_analysis, param_approx, ess_method, robust_map_object, rob_ess_method, numerical = FALSE, seed) {
  ...
  if (select_analysis == "Incidence proportion") {
    ...
    sample_prop         <- rstan::extract(map_object$fit)$theta_resp_pred
    stats_mat_prop      <- data.frame(rbind(mcmc_desc(sample_prop),
...
summary_stat_all_display <- function(select_analysis, robust_map_object, ess_method, current_trial_data, post_dist, numerical = FALSE, seed) {
  ...
  if (select_analysis == "Incidence proportion") {
    ...
    stats_mat_prop      <- data.frame(rbind(rmix_desc(robust_map_object),
                                            rmix_desc(lik),
                                            rmix_desc(post_dist)),
    ...
    stats_mat_rate      <- data.frame(rbind(rmix_desc(lik),
                                            rmix_desc(robust_map_object),
                                            rmix_desc(post_dist),
                                            mcmc_desc(robust_map_object,
                                                                   trans =TRUE, seed = seed),
    ...
    if(numerical == TRUE){
      stats_mat_rate$ESS <- c(NA, ESS_ROB,
                              NA, NA, NA)
      return(stats_mat_rate)
    }else{
      disp_mat_rate <- round(stats_mat_rate, 4)
      ...
      disp_mat_rate$ESS <- c("Not applicable.", round((ESS_ROB),1),
