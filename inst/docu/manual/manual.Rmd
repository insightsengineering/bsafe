...
When calculations were performed, you can add a card at the top left corner via \texttt{TEAL} and save your results or continue with your settings at a later stage, or log the whole R code used. Click therefor on the + (1.) in figure \ref{fig:save} and name your card.
...
In brief those three studies are presented. Each study has a \texttt{STUDYID} which corresponds in this case with its \texttt{ARM}. \texttt{Study\#1} includes historical (\texttt{HIST = 1}) and non historical (\texttt{HIST = 0}) information. It only has historical and current information for the Placebo treatment. \texttt{Study\#2} includes only current data (\texttt{HIST = 0}) and \texttt{Study\#3} only historical (\texttt{HIST = 1}).
...
It is possible to filter for each variable of the dataset. 1. (see fig. \ref{fig:teal_filter2}) select the variable you would like to filter. In this example we choose the total exposure time and the region. 2. we choose an exposure under 500 and America and Eurasia as a region.
...
Starting with 1. (see fig. \ref{fig:add_arm}) select the variables you are interested in. In this example we choose \texttt{DOSE} and \texttt{TREAT}. 2. and 3. choose the variables characteristics. In this case, we are interested to borrow information from the Placebo treatments. In general it is discouraged to borrow information from treatment arms that are not Placebo or standard of care. 4. you name the arm, and to you add the arm.
...
-   Exposure Adjusted Adverse Event Rate: sometimes referred to as Incident
    rate, which describes the incident rate of AEs per patient per time unit 
