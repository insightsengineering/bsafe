...
data set for analysis. From this section on, we will use example data (see table \ref{tab:data_example}) to demonstrate the usage of the entire application.
...
When calculations where performed, you can add a card at the top left corner via \texttt{TEAL} and save your results or to continue with your settings at a later stage, or to log the whole R code used.
...
Starting with 1. (see fig. \ref{fig:add_arm}) select the variables we are interested in. In this case we chose \texttt{DOSE} and \texttt{TREAT}. 2. and 3. choose the variables characteristics. In this case, we are interested to borrow information from the Placebo treatments. In general it is discouraged to borrow information from treatment arms that are not Placebo or standard of care. 4. you name the arm, by doing so a pop up will open and requests naming your arm and to 5. you add the arm.
By 6. a created arm can also be deleted again. Any erros that might occur (trying to give 2 times the same name), an error will be displayed at the bottom right  of you screen.
...
while you are in the same session. For more information, please refer to the statistical analysis plan. For more information how to save your analysis please and download results by creating a new card
...
A graph depicting both the MAP Prior (dotted in red) and Robust MAP Prior (dashed in blue) is the
...
For incidence proportion analysis, the uninformative mean is set to 50% and thus a Beta(1,1) distribution is chosen as the weakly-informative prior. 
...
sample size is now reduced. The prior weight correspondents with the weight for the non-informative component (2.).
...
For exposure-adjusted adverse event rate, a weakly-informative prior mean
...
For the robustification of the Exposure-Adjusted Adverse Event Rate, you need to choose a  prior weight  (1.) as well as a prior mean (2.). It is recommended to select an non-informative mean. In this case, we just used the default value 0.5, which corresponds to 3. (log(0.5) = -0.69).
...
This part serves as an exploratory option, where the robustified and borrowed information will be updated with current data.
The non-historical information from the dataset is here not used. The robustified MAP prior will be updated by the input via the sliders.

The Robust MAP Prior obtained (see section \@ref(robust-map-prior)) is then updated
with be updated by the input via the sliders. The non-historical information from the dataset is here not used. 
This provides a Bayesian Posterior. The summary of the
Robust MAP Prior, Likelihood and Posterior and thier densities are displayed. 
The generated graph allows the to  asses a potential prior data conflict.
...
Also (2.) the number of patients with at least  one event of interest observed has to be specified.
...
The Robust MAP Prior (red dotted) describes the historical.

The Likelihood describes a density that would describe current data, using the maximum likelihood estimation for an one component Beta distribution.
...
For our data example, no events observed would be a prior data conflict. In consequence,  the mean is only slightly drawn towards the MAP prior.
...
You can choose up to 5 different comparison groups. By choosing Show comparison (see fig. \ref{fig:comp select}),
...
For example could you also create an arm that includes Placebo dose 250, and add it to you selection for the control arm.



Once the Treatment and Control arms has been selected, click
the Submit button. You have then to wait for the simulation/calculation to
...
Once the simulation is completed (see fig. \ref{fig:sumi_done}), the chosen comparisons are then
