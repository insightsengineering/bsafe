---
title: "B-Safe Testing and Validation v. 0.1 for B-Safe v. 0.1"
author:
...
Test cases will cover common use cases of the application with the aim of ensuring that calculations done by the application are implemented properly. All tests are designed for \texttt{bsafe} version 0.1 in combination with the \texttt{teal.modules.bsafe} version 0.1.
...
```{r table1, echo = FALSE}
thresholds_props <- readRDS(system.file("testing_list_props.rds", package = "bsafe", mustWork = TRUE))
thresholds_rates <- readRDS(system.file("testing_list_rates.rds", package = "bsafe", mustWork = TRUE))
table <- cbind(
